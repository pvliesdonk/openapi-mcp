...
|----------|-------------|
| `OAPI_BASE_URL` | Public base URL of the server (such as `https://mcp.example.com`; include prefix when mounted under subpath, such as `https://mcp.example.com/myservice`) |
| `OAPI_OIDC_CONFIG_URL` | OIDC discovery endpoint (such as `https://auth.example.com/.well-known/openid-configuration`) |
| `OAPI_OIDC_CLIENT_ID` | OIDC client ID registered with your provider |
| `OAPI_OIDC_CLIENT_SECRET` | OIDC client secret |

## Optional Variables

| Variable | Default | Description |
|----------|---------|-------------|
| `OAPI_OIDC_JWT_SIGNING_KEY` | ephemeral | JWT signing key. **Required on Linux/Docker**: the default is ephemeral and invalidates tokens on restart |
...
| `OAPI_OIDC_VERIFY_ACCESS_TOKEN` | `false` | Set `true` to verify the upstream access token as JWT instead of the id token. Only needed when your provider issues JWT access tokens and you require audience-claim validation on that token |
...
!!! danger "Linux / Docker"
    On Linux (including Docker), the ephemeral key is especially problematic because it does not persist across process restarts. Always set `OAPI_OIDC_JWT_SIGNING_KEY` in production.
...
OAPI_OIDC_CLIENT_SECRET=your-client-secret
OAPI_OIDC_JWT_SIGNING_KEY=$(openssl rand -hex 32)
```
...
OAPI_OIDC_CLIENT_ID=my-mcp-server
OAPI_OIDC_CLIENT_SECRET=your-client-secret
OAPI_OIDC_JWT_SIGNING_KEY=your-stable-hex-key
```
...
```bash
OAPI_BASE_URL=https://mcp.example.com/myservice
OAPI_HTTP_PATH=/mcp
```
