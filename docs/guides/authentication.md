...
|------|-------------|---------------|
| **Multi-auth** | Mixed clients, such as Claude web (OIDC) + Claude Code (bearer token) on the same server | Set both `OAPI_BEARER_TOKEN` and all four OIDC variables |
| **Bearer token** | Simple deployments behind a VPN, Docker compose stacks, development | Set `OAPI_BEARER_TOKEN` only |
...
2. Set the environment variable:
    ...
    ```bash
    OAPI_BEARER_TOKEN=your-generated-token
    ```
...
The bearer-token mode above shares one subject across every authenticated caller. By default this is the library's `bearer-anon`; override with `OAPI_BEARER_DEFAULT_SUBJECT`. For audit logs and authorization that distinguish callers, switch to mapped-token mode by pointing `OAPI_BEARER_TOKENS_FILE` at a TOML file:
...
|----------|-------------|
| `OAPI_BASE_URL` | Public base URL (such as `https://mcp.example.com`) |
| `OAPI_OIDC_CONFIG_URL` | OIDC discovery endpoint |
| `OAPI_OIDC_CLIENT_ID` | Client ID registered with your provider |
| `OAPI_OIDC_CLIENT_SECRET` | Client secret |

### Optional variables

| Variable | Default | Description |
|----------|---------|-------------|
| `OAPI_OIDC_JWT_SIGNING_KEY` | ephemeral | JWT signing key (**required on Linux/Docker**) |
| `OAPI_OIDC_AUDIENCE` | n/a | Expected JWT audience claim; leave unset if your provider does not set one |
| `OAPI_OIDC_REQUIRED_SCOPES` | `openid` | Comma-separated required scopes |
| `OAPI_OIDC_VERIFY_ACCESS_TOKEN` | `false` | Set `true` to verify the access token as a JWT instead of the id token; useful for audience-claim validation on JWT access tokens |
...
You're missing `OAPI_OIDC_JWT_SIGNING_KEY`. Without it, FastMCP generates an ephemeral key on each startup. Generate and set a stable key:
