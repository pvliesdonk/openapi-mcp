...
echo "=== openapi-mcp .deb install test ==="

# Find the .deb
DEB=$(find dist/ -name 'openapi-mcp*.deb' -print -quit 2>/dev/null || true)
if [ -z "$DEB" ]; then
...
if getent passwd openapi-mcp >/dev/null 2>&1; then
    pass "System user 'openapi-mcp' exists"
else
    fail "System user 'openapi-mcp' not found"
fi

# 2. System group exists
if getent group openapi-mcp >/dev/null 2>&1; then
    pass "System group 'openapi-mcp' exists"
else
    fail "System group 'openapi-mcp' not found"
fi

# 3. State directory exists with correct ownership and permissions
if [ -d /var/lib/openapi-mcp ]; then
    OWNER=$(stat -c '%U:%G' /var/lib/openapi-mcp)
    MODE=$(stat -c '%a' /var/lib/openapi-mcp)
    if [ "$OWNER" = "openapi-mcp:openapi-mcp" ] && [ "$MODE" = "750" ]; then
        pass "State directory owned by openapi-mcp with mode 0750"
    elif [ "$OWNER" != "openapi-mcp:openapi-mcp" ]; then
        fail "State directory owned by $OWNER (expected openapi-mcp:openapi-mcp)"
    else
        fail "State directory mode $MODE (expected 750)"
    fi
else
    fail "/var/lib/openapi-mcp does not exist"
fi

# 4. Install directory exists
if [ -d /opt/openapi-mcp ]; then
    pass "Install directory /opt/openapi-mcp exists"
else
    fail "Install directory /opt/openapi-mcp does not exist"
fi

# 4b. Venv installed by postinstall
if [ -x /opt/openapi-mcp/venv/bin/python3 ]; then
    pass "venv installed at /opt/openapi-mcp/venv"
else
    fail "venv not found at /opt/openapi-mcp/venv (postinstall may have failed)"
fi

# 5. Unit file exists
if [ -f /usr/lib/systemd/system/openapi-mcp.service ]; then
...
if command -v systemd-analyze >/dev/null 2>&1; then
    if output=$(systemd-analyze verify /usr/lib/systemd/system/openapi-mcp.service 2>&1); then
